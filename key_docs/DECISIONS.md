...
## ADR-030: 멀티유저 아티클 공유 — 복사 방식 채택

**일시**: 2026-03-12
**상태**: 확정

**문제**: 유저가 마스터와 동일한 소스를 등록하면, `UNIQUE(source_id)` 제약으로 크롤링된 아티클이 INSERT 실패. 유저 마이피드에 콘텐츠가 전혀 표시되지 않음.

**결정**: 아티클 복사 방식(Option A)을 채택.
1. DB: `UNIQUE(source_id)` → `UNIQUE(source_id, user_id)` — 같은 아티클을 유저별로 보유 가능
2. `runCrawler`: 크롤링 전 다른 유저의 동일 소스 아티클이 있으면 통째 복사 (제목, 요약, 태그 등) → 크롤링/AI 요약 완전 스킵
3. `saveArticles`: 중복 체크에 `user_id` 스코핑 추가

**대안 검토**:
| 방식 | 장점 | 단점 | 결정 |
|------|------|------|------|
| **A. 아티클 복사** | 변경 범위 작음, 기존 쿼리 유지 | row 복제 (유저 수 × 아티클) | **채택** |
| **B. 공유 아티클** (user_id 제거) | 중복 0, 스토리지 최적 | 조회/삭제/필터 쿼리 전면 리팩토링 | 보류 |

**이유**:
- 현재 articles에 `user_id` NOT NULL 의존이 깊음 (RLS, API 쿼리, 캐시 키 등)
- 유저 수가 적은 초기 단계에서 row 복제 비용은 미미
- 크롤링/AI API 비용 절감이 핵심 — 복사는 DB INSERT 1회로 완료
- 향후 유저 규모 커지면 Option B로 마이그레이션 가능
