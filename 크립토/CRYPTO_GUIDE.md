...
├── CRYPTO_PIPELINE.md       ← 3-크론 데이터 파이프라인 + 프론트엔드 플로우
├── CRYPTO_FILES.md          ← 파일 구조 (생성/수정 파일 목록)
├── CRYPTO_INFRA.md          ← 환경변수 + Edge Functions + 토큰 + DB 스키마 + RLS
├── CRYPTO_SCORING.md        ← 시그널 가중치 V2 + 시각화 상세
├── CRYPTO_SOURCES.md        ← 데이터 소스 상세 (Reddit/Twitter/Telegram/4chan/DexScreener)
├── CRYPTO_CHECKLIST.md      ← Phase별 검증 체크리스트
├── 진행중/
│   ├── ONTOLOGY_GUIDE.md              ← 온톨로지 아키텍처 + KG 비례 스코어링
│   └── BACKTEST_TREND_CHART_PLAN.md   ← 백테스트 추이 차트 + KG 검증 체크리스트
├── 비즈니스/
│   └── PROJECT_VALIDITY.md            ← 프로젝트 타당성 + 학술 근거
├── 참고문서/
│   └── BATTLE_TRADING_RULES.md        ← AI 배틀 트레이딩 룰
└── 히스토리/
    ├── CRYPTO_PREDICTOR_HANDOFF.md    ← 초기 핸드오프 문서
    └── DATA_SOURCE_EXPANSION_PLAN.md  ← 데이터소스 확장 제안서 (Phase J~L)
```
...
| "점수 계산 로직 변경" | SCORING (가중치), 진행중/ONTOLOGY_GUIDE (KG 부스트) |
| "새 데이터소스 추가" | SOURCES |
