...
export default async function TagPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    '@type': 'CollectionPage',
    name: `#${name} - 태그별 아티클`,
    url: `https://aca-info.com/tags/${encodeURIComponent(name)}`,
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      />
      <SeoBreadcrumb items={[{ label: '태그', href: '/tags' }, { label: `#${name}` }]} />
          ...
          <div className="sticky top-8">
            ...
            <div className="flex flex-col gap-1.5">
              {categories.map((c) => (
                <Link
                  key={c.name}
                  href={`/topics/${encodeURIComponent(c.name)}`}
            ...
            <div className="mt-6 pt-4 border-t border-[var(--border)]">
              <Link href="/tags" className="text-sm text-[var(--accent)] hover:underline">← 전체 태그</Link>
            </div>
          </div>
        </aside>
      </div>
