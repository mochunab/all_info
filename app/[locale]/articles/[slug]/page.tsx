...
export default async function ArticlePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    url: `https://aca-info.com${lp(`/articles/${slug}`)}`,
  ...
  const breadcrumbItems = [
    ...(article.category
      ? [{ label: article.category, href: lp(`/topics/${encodeURIComponent(article.category)}`) }]
      : []),
    { label: title },
  ];
        ...
        <article>
          <header className="mb-8 pb-6 border-b border-[var(--border)]">
            ...
            <div className="flex items-center gap-3 flex-wrap text-sm text-[var(--text-tertiary)]">
              ...
              {article.author && (
                <Link
                  href={lp(`/authors/${encodeURIComponent(article.author)}`)}
