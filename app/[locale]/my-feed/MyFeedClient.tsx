...
export default function MyFeedClient({
  ...
  const fetchArticles = useCallback(
    ...
    [userId, search, category, lastUpdated, articles.length, categories]
  );

  // Revalidate categories (background, like HomeFeed)
  useEffect(() => {
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.categories && data.categories.length > 0) {
            ...
            try {
              sessionStorage.setItem(STORAGE_KEY.MY_CATEGORIES, JSON.stringify({ data: categoryNames, translations: data.categories, timestamp: Date.now() }));
            } catch { /* ignore */ }
          }
        }
      } catch (error) {
  ...
  // Sync categories when returning from source management
  useEffect(() => {
    try {
      const cached = sessionStorage.getItem(STORAGE_KEY.MY_CATEGORIES);
      if (!cached) return;
  ...
  // Fetch articles on category/search change (same pattern as HomeFeed)
  useEffect(() => {
    if (!category) return;
    ...
    fetch('/api/crawl/trigger', {
      ...
      .catch((error) => {
        ...
        if ((error as Error).name === 'AbortError') {
          ...
          abortParams.set('user_id', userId);
