...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/${locale}/authors`, languages: buildAlternateLanguages('/authors') },
    openGraph: {
...
export default async function AuthorsPage({
  ...
  const jsonLd = {
    ...
    url: `https://aca-info.com${lp('/authors')}`,
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      />
      <SeoBreadcrumb items={[{ label: '저자' }]} locale={locale} />
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {authors.map((a) => (
          <Link
            key={a.name}
            href={lp(`/authors/${encodeURIComponent(a.name)}`)}
