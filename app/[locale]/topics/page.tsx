...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/${locale}/topics`, languages: buildAlternateLanguages('/topics') },
    openGraph: {
...
export default async function TopicsPage({
  ...
  const jsonLd = {
    ...
    url: `https://aca-info.com${lp('/topics')}`,
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      />
      <SeoBreadcrumb items={[{ label: '토픽' }]} locale={locale} />
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {categories.map((cat) => (
          <Link
            key={cat.name}
            href={lp(`/topics/${encodeURIComponent(cat.name)}`)}
