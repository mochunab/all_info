...
const EDGE_FUNCTION_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
  ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/analyze-crypto-sentiment`
  : null;
...
export async function processCryptoSentiments(
  ...
  try {
    ...
    if (error || !postsData) {
      ...
      return processPostBatch(supabase, fallbackData as PostRow[], result);
    }
    ...
    return processPostBatch(supabase, postsData as PostRow[], result);
  } catch (error) {
      ...
      chunk.map(async (post) => {
        const aiResult = await withRetry(async () => {
          if (USE_EDGE_FUNCTION && supabaseKey) {
            const res = await analyzeViaEdgeFunction(post.title, post.body, supabaseKey);
